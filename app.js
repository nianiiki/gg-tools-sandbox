...
function screenDistributor(){
  ...
  view.querySelector('[data-act="share"]')?.addEventListener("click", ()=>{
    ...
    if(!sid) return toast("No live session.");
    const playerUrl = new URL(window.location.href);
    playerUrl.hash = `#/distributor/player?sid=${encodeURIComponent(sid)}&cap=${encodeURIComponent(cap)}`;
    shareText(`GG Promo Code Claim`, `Scan to claim your code:\n${playerUrl.toString()}`);
  });
